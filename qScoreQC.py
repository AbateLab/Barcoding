...
"""Judge reads by Percent of read above certain phred
score, max phred score allowed, and Expected Number
of Incorrect Bases, calculated by the probabilities 
of individual bases being incorrect. This value can
lie between 0 (tell me how you did it), and the length
of your read (god help you). eval() actually does all
the hard calc's O(#reads * len(read)), the other
functions run over logs generated by eval O(len(read)
+ something), and may be run with different parameters
multiple times after eval has generated the logs"""
...
def main():
    if args.eval:
        if args.input:
            ...
            print "eval requires -i to run. -b defaults to 20, -bc defaults to 22, -pc defaults to 30"
            return
    if args.filter:
        if args.out:
            filterq()
        else:
            print "filter requires -o to run. -fo defaults to 0, -n defaults to 1000,\
            -r defaults to .001"
    if args.graph:
        seqEndN()
        baseComposition()
...
def filterq():
    ...
    with open(args.folder + "/nReplace", "r") as f, open(args.folder + "/log", "r") as log,\
        ...
        for lab, seq, exp in readfx(f):
            ...
            if n < args.ncutoff:
                n = 0
            if float(l[args.fopt]) < args.readcutoff:
    ...
    if scores:
        sHist(scores, ["Expected Incorrect Bases per Read", "Maximum Single Base Error", 
            "Percent of Read > pcut"][args.fopt], False, args.out)
    else:
...
def scoreDistribution():
    ...
    sHist(pcp, "Percent of Read > pcut", True, 'pcpHist')
